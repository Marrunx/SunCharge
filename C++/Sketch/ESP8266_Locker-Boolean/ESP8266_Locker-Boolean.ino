...
void getBalanceFromServer(String uid) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      // Parse response (e.g., "100,1" for balance 100 and access true)
      int commaIndex = response.indexOf(','); // Find the comma separator
      String balanceStr = response.substring(0, commaIndex); // Extract the balance
      String accessStr = response.substring(commaIndex + 1); // Extract the access

      int rfid_balance = balanceStr.toInt();     // Convert balance to an integer
      bool access = (accessStr == "1");          // Convert access to a boolean

      Serial.println("RFID Balance: " + String(rfid_balance));
      Serial.println("Access: " + String(access ? "true" : "false"));

      if (rfid_balance > 0 || access) {
        // Log APPROVE description
        logAccessToServer(uid, "This UID has access to Locker 1, APPROVE");

        // Send the RFID balance to the coinslot_balance column
        sendBalanceToCoinslotServer(uid, rfid_balance, access);
        ...
        // Log DENIED description
        logAccessToServer(uid, "This UID is trying to access Locker 1, DENIED");
      }
    } else {
      Serial.println("Error on sending POST: " + String(httpResponseCode));
    }
...
void sendBalanceToCoinslotServer(String uid, int rfid_balance, bool access) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    ...
    // Prepare data
    String httpRequestData = "card_uid=" + uid + 
                             "&rfid_balance=" + String(rfid_balance) + 
                             "&access=" + String(access ? "1" : "0"); // Add boolean as 1 (true) or 0 (false)
